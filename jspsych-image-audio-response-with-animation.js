/**
 * jspsych-image-audio-response-with-animation
 * based on "jspsych-image-audio-response" (https://github.com/becky-gilbert/audio-response-timing/blob/master/experiments/jspsych-image-audio-response.js)
 * 
 **/

jsPsych.plugins["image-audio-response-with-animation"] = (function() {

    let plugin = {};

    jsPsych.pluginAPI.registerPreload('image-audio-response-with-animation', 'stimulus', 'image');

    plugin.info = {
        name: 'image-audio-response-with-animation',
        ...
        parameters: {
            ...
            recording_animation_keyframes: {
                ...
                pretty_name: 'Recording animation keyframes',
                default: [
                    { offsetDistance: '0%' },
                    { offsetDistance: '100%' }
                  ],
                description: 'an array of keyframes that animate "recording_animation_id"; see https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Keyframe_Formats',
                array: true
            },
            recording_animation_id: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Recording animation ID',
                default: 'jspsych-image-audio-response-light',
                description: 'The recording element ID that receives animation'
            },
        }
    };
        ...
        detect_mic(function(has_mic) {
            ...
            if (!has_mic) {
                ...
                function process_audio(stream) {
                    ...
                    recorder.start(1000);
                    document.getElementById(trial.recording_animation_id).animate(
                        trial.recording_animation_keyframes, trial.buffer_length).onfinish = function() {
